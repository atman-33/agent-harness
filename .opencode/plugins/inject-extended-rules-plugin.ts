...
// them to files in ANY repo via cwd-relative globs (`paths: ../repo/**`). It does
// not depend on `.claude/project-context.json`.
//
  ...
  return {
    "tool.execute.before": async (input, output) => {
      ...
      for (const touchedPath of touchedPaths) {
        const relativePath = toCwdRelativePath(touchedPath, workspaceRoot);
        if (!relativePath) {
          continue;
        }

        queueExtendedRules(workspaceRoot, relativePath, state);
      }
    },
...
function queueExtendedRules(
  ...
  for (const rule of loadExtendedRules(workspaceRoot, relativePath)) {
