# PR Assistant Reference

This reference expands the `pr-assistant` workflow for `agent-harness`.
...
$HarnessRoot = "C:\repos\agent-harness"
$TargetRepo = "C:\repos\multi-agent-ff15-vscode"
$TempRoot = Join-Path $HarnessRoot ".github\skills\pr-assistant\temp-files"
...
python "$HarnessRoot\.github\skills\pr-assistant\scripts\analyze_changes.py" "$TargetBranch" --output "$AnalysisFile"
```
...
```powershell
$TemplateFile = "$HarnessRoot\.github\skills\pr-assistant\assets\templates\pr-template-feature-ja.md"

python "$HarnessRoot\.github\skills\pr-assistant\scripts\generate_pr_body.py" `
  "$TemplateFile" `
  "$AnalysisFile" `
  "$PrBodyFile" `
  --language ja
...
```powershell
python "$HarnessRoot\.github\skills\pr-assistant\scripts\quality_checks.py" "$TargetBranch"
```
